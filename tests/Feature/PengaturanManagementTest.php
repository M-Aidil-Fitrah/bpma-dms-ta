...
final class PengaturanManagementTest extends TestCase
{
    ...
    public function test_superadmin_dapat_menyimpan_ketiga_pengaturan_yang_diizinkan(): void
    {
        $this->actingAs($this->superadmin)
            ->patch('/admin/settings', [
                'unggah_batas_kb' => 2048,
                'dokumen_per_halaman' => 10,
                'dokumen_rentang_evaluasi_awal' => 7,
            ])
            ->assertRedirect()
        ...
        $this->assertSame(7, $service->integer('dokumen.rentang_evaluasi_awal'));
    ...
    public function test_pengaturan_tidak_menerima_nilai_di_luar_batasnya(): void
    {
        $this->actingAs($this->superadmin)
            ->patch('/admin/settings', [
                ...
                'dokumen_rentang_evaluasi_awal' => 13,
            ])
            ->assertSessionHasErrors(['unggah_batas_kb', 'dokumen_per_halaman', 'dokumen_rentang_evaluasi_awal']);
    }
    ...
    public function test_nilai_kosong_menghapus_override_dan_kembali_ke_bawaan(): void
    {
        ...
        $this->actingAs($this->superadmin)
            ->patch('/admin/settings', [
                ...
                'dokumen_rentang_evaluasi_awal' => null,
            ])
            ->assertSessionHasNoErrors();
    ...
    public function test_pengaturan_rentang_awal_dipakai_dasbor_tanpa_query_string(): void
    {
        ...
        $this->actingAs($this->superadmin)->get('/dashboard')
            ->assertInertia(fn (AssertableInertia $page) => $page->where('data.rentang_evaluasi', 7));
    }
}
